...
def get_classification_logits_cache(self, x, text_classifier, logit_scale, num_templates=None):
        # x in shape of [B, *, C]
        # text_classifier in shape of [num_classes, C]
        # logit_scale is a learnable scalar https://github.com/mlfoundations/open_clip/blob/main/src/open_clip/model.py#L201
        # return: [B, *, num_classes]
        ...
        affinity = x @ self.cache_keys.T
        cache_logits = logit_scale * F.softmax(affinity, dim=2) @ self.cache_values.to(torch.float32)
        if self.generalized:
            final_pred_logits[:, :, :15]  = final_pred_logits[:, :, :15] + cache_logits * self.alpha
        else: 
          final_pred_logits[:, :, :4]  = final_pred_logits[:, :, :4] + cache_logits[:, :, [3, 9, 10, 12]] * self.alpha
        return final_pred_logits
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.indices = torch.load(channel_indices)
        self.dec = dec
    @classmethod
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["clip_embedding_dim"] = cfg.MODEL.ZORI.EMBED_DIM
        ret['channel_indices'] = cfg.MODEL.CHANNEL_INDICES
        ret['dec'] = cfg.MODEL.DEC
        return ret
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, text_classifier, num_templates):
        ...
        if self.dec:
            class_embed = class_embed[:, :, self.indices]
            text_classifier = text_classifier[:, self.indices]
