...
class MaskComparisonVisualizer:
    ...
    def _get_category_color(self, category_id):
        ...
        if category_id not in self.category_colors:
            # 按类别ID分配颜色（循环使用）
            color_idx = len(self.category_colors) % len(self.predefined_colors)
            self.category_colors[category_id] = self.predefined_colors[color_idx]
    ...
    def visualize_image(self, image_id, score_threshold=0.3, add_text=True):
        ...
        if add_text:
            ...
            comparison[text_height:, w:] = vis_img
...
if __name__ == "__main__":
    ...
    # 创建可视化器
    print("初始化对比可视化器...")
    visualizer = MaskComparisonVisualizer(
    ...
    visualizer.visualize_all(
        max_images=300,  # 可视化前50张
        score_threshold=0.1,  # 置信度阈值
        add_text=True  # 添加标题文字
    )

    # 或者可视化特定图像
    # visualizer.visualize_image(image_id=1, score_threshold=0.3, add_text=True)import json
    # import cv2
    # import numpy as np
    # import os
    # from pathlib import Path
